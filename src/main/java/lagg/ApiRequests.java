...
public class ApiRequests {
	...
	public static void waitUntilHypixelCooldownOver() {
	   ...
	   public static String getPlayerResponse(UUID user) {
	    ...
	    public static String[] getUsernames(UUID user) {
	    	waitUntilMojangCooldownOver();
			String response = "";
		...
		public static Object[][] getBlitzKits(String apiResponse) {
			ArrayList<Object[]> list = new ArrayList<Object[]>();
			Matcher m1 = Pattern.compile("\"HungerGames\":\\{(.*?)\"togglekillcounter\":").matcher(apiResponse);
	        m1.find();
			Matcher m = Pattern.compile("\"([a-z ]+)\":(\\d),").matcher(m1.group(1));
	        while(m.find()) {
	        	Object[] array = {m.group(1),Integer.parseInt(m.group(2))+1};
	        	list.add(array);
	        }
	        Object[][] arraybig = new Object[list.size()][2];
	        for(int i = 0; i<list.size(); i++) {
	        	arraybig[i][0] = list.get(i)[0];
	        	arraybig[i][1] = list.get(i)[1];
	        }
	        return arraybig;
		}
